"""Single-command launcher for the live paper cockpit."""
...
def _start_live_engine(log_dir: Path) -> ServiceHandle:
    python = sys.executable
    env = os.environ.copy()
...
def _monitor(services: list[ServiceHandle], log_dir: Path, ports: RuntimePorts) -> None:
    ...
    while True:
        ...
        if time.time() - last_heartbeat >= 15.0:
            print(
                f"[{datetime.now().strftime('%H:%M:%S')}] cockpit healthy | "
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Launch the full live paper trading cockpit.")
...
def main() -> None:
    ...
    log_dir = ROOT / "live_sim" / "output" / f"cockpit_launcher_{_timestamp()}"
    log_dir.mkdir(parents=True, exist_ok=True)

    services: list[ServiceHandle] = []
    try:
        services.append(_start_live_engine(log_dir))
