...
def build_h6_moonshot_snapshots(execution_index, df_6h, df_12h=None, df_1d=None, *, config=None):
    ...
    raw = getter("strategy", "h6_moonshot", default={}) if callable(getter) else {}
    raw = raw or {}
...
class H6MoonshotEngine:
    ...
    def build_candidate(self, *, symbol, timestamp, execution_row, snapshot, momentum_rank, top_symbols):
        del top_symbols
        if not self.enabled or not snapshot or not _bool_value(snapshot.get("h6_new_candle")):
            return None
        ...
        return {
            ...
            "selection_score": clamp(0.85 * raw_score + 0.15 * float(momentum_rank or 0.0)),
            ...
            "execution_profile": {
                ...
                "max_hold_candles": self.max_hold_6h_candles * 24,
            },
