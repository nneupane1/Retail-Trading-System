...
class EqualHighsLiquiditySweepRescueForensicAuditTests(unittest.TestCase):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            result = write_equal_highs_liquidity_sweep_rescue_forensic_audit(
                EqualHighsLiquiditySweepRescueForensicAuditConfig(
                    ...
                    output_root=output_root / "equal_highs_liquidity_sweep_rescue_forensic_audit_001",
                )
            )
            ...
            self.assertIn(summary["final_classification"], {
                "RESCUE_REJECTED",
                "RESCUE_WEAK_UNSTABLE",
                "RESCUE_PROMISING_BUT_COST_FRAGILE",
                "RESCUE_PROMISING_BUT_MOONSHOT_FRAGILE",
                "RESCUE_PROMISING_RESEARCH_ONLY",
                "RESCUE_STRONG_RESEARCH_CANDIDATE",
                "RESCUE_READY_FOR_NATIVE_ENGINE_REPRODUCTION_RESEARCH_ONLY",
            })

            diagnostics_root = output_root / "equal_highs_liquidity_sweep_rescue_forensic_audit_001" / "diagnostics"
            reconstruction = json.loads((diagnostics_root / "rescue_candidate_reconstruction.json").read_text(encoding="utf-8"))
            no_leak = json.loads((diagnostics_root / "rescue_candidate_no_future_leakage_check.json").read_text(encoding="utf-8"))
            missing_fields = json.loads((diagnostics_root / "missing_pre_entry_fields_report.json").read_text(encoding="utf-8"))
            governors = json.loads((diagnostics_root / "risk_governor_impact.json").read_text(encoding="utf-8"))
            strict_no_leak = json.loads((diagnostics_root / "stricter_rescue_variant_no_leakage_check.json").read_text(encoding="utf-8"))

            self.assertEqual("RESCUE_EQUAL_HIGHS_LIQUIDITY_SWEEP", reconstruction["candidate_name"])
            self.assertTrue(no_leak["final_no_leakage_verdict"])
            self.assertIn("missing_pre_entry_fields", missing_fields)
            self.assertIn("governor_variants", governors)
            self.assertTrue(strict_no_leak["final_no_leakage_verdict"])

            with (diagnostics_root / "rescued_short_trade_profile.csv").open("r", encoding="utf-8") as handle:
                list(csv.DictReader(handle))
            with (diagnostics_root / "rescued_short_winner_vs_loser_contrast.csv").open("r", encoding="utf-8") as handle:
                list(csv.DictReader(handle))
            with (diagnostics_root / "rescue_candidate_rolling_5y_robustness.csv").open("r", encoding="utf-8") as handle:
                self.assertTrue(list(csv.DictReader(handle)))
            with (diagnostics_root / "stricter_rescue_variant_results.csv").open("r", encoding="utf-8") as handle:
                strict_rows = list(csv.DictReader(handle))
            self.assertTrue(strict_rows)
            self.assertTrue(all(row["pre_entry_safety_status"] == "True" for row in strict_rows))
