"""Checkpoint-safe candidate and execution validation for the dormant 6H bridge sleeve."""
...
def _report_root(base: AppConfig) -> Path:
    return Path(base.require("backtest", "output_dir")) / "h6_moonshot_validation_current"
...
def _build_h6_research_timeframes(df_1m: pd.DataFrame, config: AppConfig):
    builder = TimeframeBuilder(config=config)
    execution_rule = config.require("timeframes", "execution", "rule")
    bridge_rule = config.require("timeframes", "bridge", "rule")
    ...
    with _suppress_pipeline_output():
        ...
        df_6h = builder.resample(df_1m, bridge_rule)
    ...
    return df_15m, df_6h, df_12h, df_1d
...
def _simulate_h6_trade(
    *,
    ...
    max_hold_bars = int(engine.max_hold_6h_candles) * 24
    ...
    return {
        ...
        "side": "long",
        "signal_family": str(snapshot.get("signal_family_long") or "h6_bridge_breakout"),
        ...
        "h6_score_long": _safe_float(snapshot.get("h6_score_long"), default=0.0),
        "h6_range_expansion": _safe_float(snapshot.get("h6_range_expansion"), default=0.0),
        "h6_body_strength": _safe_float(snapshot.get("h6_body_strength"), default=0.0),
        "h6_close_position": _safe_float(snapshot.get("h6_close_position"), default=0.0),
        "h6_context_12h": str(snapshot.get("h6_context_12h") or "neutral"),
        "h6_context_1d": str(snapshot.get("h6_context_1d") or "neutral"),
    }
...
def _validate_symbol(
    *,
    ...
    df_15m, df_6h, df_12h, df_1d = _build_h6_research_timeframes(df_1m, config=config)
    ...
    h6_steps = recent_snapshots.loc[
        recent_snapshots["h6_new_candle"].fillna(False).astype(bool)
    ].copy()

    funnel = {
        "symbol": str(symbol).upper(),
        "source_path": str(source_path),
        "raw_6h_events": int(len(h6_steps)),
        "passed_structure": int(h6_steps["h6_pass_structure_long"].fillna(False).astype(bool).sum()),
        "passed_shape": int(h6_steps["h6_pass_shape_long"].fillna(False).astype(bool).sum()),
        "passed_12h_context": int(h6_steps["h6_pass_12h_context_long"].fillna(False).astype(bool).sum()),
        "passed_1d_context": int(h6_steps["h6_pass_1d_context_long"].fillna(False).astype(bool).sum()),
        "passed_score": int(
            (
                pd.to_numeric(h6_steps["h6_score_long"], errors="coerce").fillna(0.0)
                >= float(engine.raw.get("min_score", 0.76))
            ).sum()
        ),
        "opened_candidates": int(h6_steps["signal_event_long"].fillna(False).astype(bool).sum()),
    }

    event_rows = []
    mask = h6_steps["signal_event_long"].fillna(False).astype(bool)
    for timestamp in h6_steps.index[mask]:
        snapshot = h6_steps.loc[timestamp].to_dict()
        execution_row = recent_exec.loc[timestamp]
        trade_row = _simulate_h6_trade(
            symbol=symbol,
            timestamp=timestamp,
...
def _write_reports(report_root: Path, funnel_rows: list[dict], event_rows: list[dict], symbols: list[str]) -> dict:
    ...
    events_df = pd.DataFrame(event_rows).sort_values(["timestamp", "symbol"]).reset_index(drop=True) if event_rows else pd.DataFrame()
    summary = {
    ...
    funnel_path = report_root / "h6_funnel_by_symbol.csv"
    funnel_df.to_csv(funnel_path, index=False)
    summary["funnel_by_symbol_report"] = str(funnel_path)

    events_path = report_root / "h6_event_trades.csv"
    if not events_df.empty:
        ...
        by_symbol_path = report_root / "h6_event_summary_by_symbol.csv"
        ...
        by_year_path = report_root / "h6_event_summary_by_year.csv"
