"""Lean holdout validation for the research-only 1H execution sleeve."""
...
from backtest.validate_h1_execution import _clone_config, _events_metrics, _validate_symbol
from config import AppConfig
from entry.h1_execution import H1ExecutionEngine
...
MIN_KEEP_TRADE_COUNT = 40
MIN_KEEP_NET_R = 0.0
MIN_KEEP_PROFIT_FACTOR = 1.05


def _report_root(base: AppConfig) -> Path:
    return Path(base.require("backtest", "output_dir")) / "h1_execution_holdout_current"
...
def main():
    ...
    pd.DataFrame(training_funnels).sort_values("symbol").to_csv(
        report_root / "training_h1_funnel_by_symbol.csv",
        index=False,
    )
    pd.DataFrame(holdout_funnels).sort_values("symbol").to_csv(
        report_root / "holdout_h1_funnel_by_symbol.csv",
        index=False,
    )
    training_summary_df.to_csv(report_root / "training_h1_summary_by_symbol.csv", index=False)
    holdout_summary_df.to_csv(report_root / "holdout_h1_summary_by_symbol.csv", index=False)
    classification_df.to_csv(report_root / "training_h1_symbol_curation.csv", index=False)
    if not training_events_df.empty:
        training_events_df.to_csv(report_root / "training_h1_event_trades.csv", index=False)
    if not holdout_events_df.empty:
        holdout_events_df.to_csv(report_root / "holdout_h1_event_trades.csv", index=False)
    ...
    summary = {
        ...
        "training_symbol_curation": {
            ...
            "report": str(report_root / "training_h1_symbol_curation.csv"),
        },
